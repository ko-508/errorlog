...
Bitbucket の 500 [エラー](/glossary/エラー/)は、Bitbucket [サーバー](/glossary/サーバー/)側で予期しない内部[エラー](/glossary/エラー/)が発生したことを示します。[クライアント](/glossary/クライアント/)側の問題ではなく、Bitbucket のインフラストラクチャや[リポジトリ](/glossary/リポジトリ/)処理中に異常が生じた状態です。この[エラー](/glossary/エラー/)が発生すると、[プッシュ](/glossary/プッシュ/)、プルリクエストの作成、リポジトリクローンなどの基本的な [Git](/glossary/git/) 操作が失敗し、開発フローが中断されます。

## 実際のエラーメッセージ例

Bitbucket Web UI でアクセスした場合の[レスポンス](/glossary/レスポンス/)：
...
[Git](/glossary/git/) [コマンドライン](/glossary/コマンドライン/)実行時の[エラー](/glossary/エラー/)：
...
Bitbucket のサーバーシステムやクラウドインフラで一時的な障害が発生している状態です。Atlassian のシステムメンテナンス中、[ネットワーク](/glossary/ネットワーク/)障害、または[データベース](/glossary/データベース/)接続の問題が原因となります。このような障害は通常、Atlassian 側で自動的に復旧されますが、復旧までの間はすべてのユーザーに影響します。
...
[リポジトリ](/glossary/リポジトリ/)のパッケージファイル（`.pack` [ファイル](/glossary/ファイル/)）の読み書き、ガベージコレクション処理、または大容量[ファイル](/glossary/ファイル/)の処理中に Bitbucket [サーバー](/glossary/サーバー/)が内部[エラー](/glossary/エラー/)に遭遇する場合があります。特に、大規模な[コミット](/glossary/コミット/)履歴を持つ[リポジトリ](/glossary/リポジトリ/)や、バイナリファイルを多く含む[リポジトリ](/glossary/リポジトリ/)で発生しやすい傾向があります。
...
ローカル[環境](/glossary/環境/)からの接続、企業[ファイアウォール](/glossary/ファイアウォール/)経由の接続、または VPN 接続時に、中間[プロキシ](/glossary/プロキシ/)やゲートウェイが Bitbucket との[通信](/glossary/通信/)を正しく処理できず、[サーバー](/glossary/サーバー/)から 500 [エラー](/glossary/エラー/)が返される場合があります。リトライロジックの不備や [TLS](/glossary/tls/) ハンドシェイク（暗号化通信開始時の処理）の失敗も原因となります。
...
また、Bitbucket Pipelines を使用している場合、パイプライン実行中に 500 [エラー](/glossary/エラー/)が発生することもあります。この場合、[リポジトリ](/glossary/リポジトリ/)の[設定](/glossary/設定/)（プロジェクトキー、[環境変数](/glossary/環境変数/)）を確認し、Pipelines の実行[ログ](/glossary/ログ/)から詳細なスタックトレースを取得することが重要です。
...
500 [エラー](/glossary/エラー/)が 30 分以上続く場合、またはステータスページに障害報告がない場合は、Atlassian サポートに直接問い合わせてください。その際、以下の情報を収集して提供することが重要です：
...
上記の[ログ](/glossary/ログ/)と、以下の情報を含めてサポートチケットを作成してください：
- [リポジトリ](/glossary/リポジトリ/)の [URL](/glossary/url/)
- 発生した [Git](/glossary/git/) [コマンド](/glossary/コマンド/)（push、pull、clone など）
- [エラー](/glossary/エラー/)が発生した時刻（UTC）
- `git-debug.log` の内容
- ローカル [Git](/glossary/git/) [バージョン](/glossary/バージョン/)（`git --version`）
- [ネットワーク](/glossary/ネットワーク/)[環境](/glossary/環境/)（[プロキシ](/glossary/プロキシ/)の有無、VPN 使用状況）
