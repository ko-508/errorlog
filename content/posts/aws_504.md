...
接続の失敗や不正な応答は 502 です（[AWS の 502 エラー](/posts/aws_502/)）。ターゲットグループにターゲットが登録されていない場合は 503 です（[AWS の 503 エラー](/posts/aws_503/)）。また、CloudFront などさらに手前の中継役を挟んでいる場合、手前のタイマーのほうが短ければ504を返すのは手前側になります。多段構成では、外側ほどタイマーが長くなる（クライアント > CloudFront > ALB > アプリ内の処理タイムアウト）ように整合させると、時間切れの発生場所が安定し、調査が楽になります。
