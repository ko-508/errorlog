...
問題が特定の [SDK](/glossary/sdk/) バージョンやサービスに限定される場合は、リージョンの状態ページ（https://status.azure.com）も確認し、Azure 側のインシデント有無を確認してください。
