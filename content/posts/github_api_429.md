...
同じレート制限の超過でも、403 Forbidden として返る場合があります（公式仕様。x-ratelimit-remaining が 0 か、message がレート制限系かで見分けられます。対処はこの記事と同じです。[403 の記事](/posts/github_api_403/)）。一方、Bad credentials の 401 はトークン自体の問題であり、レート制限とは無関係です（[401 の記事](/posts/github_api_401/)）。
