...
データベースへのアクセスが集中し、プロビジョニングされたキャパシティ（RDS の接続数、DynamoDB の読み取り/書き込み容量）を超えると、[バックエンド](/glossary/バックエンド/)が 503 を返します。
...
- **ターゲットグループの[ヘルスチェック](/glossary/ヘルスチェック/)失敗**：ヘルスチェックパス、プロトコル、[ポート](/glossary/ポート/)番号が正しいか確認。例えば、`/health` [エンドポイント](/glossary/エンドポイント/)が存在しない場合、すべてのターゲットが unhealthy になります。
...
- **Lambda 関数のエラー**：Lambda の[タイムアウト](/glossary/タイムアウト/)（デフォルト3秒）、メモリ不足、または実行[ロール](/glossary/ロール/)（[IAM](/glossary/iam/) Role）の権限不足により、関数が失敗します。CloudWatch ログを確認してください。
