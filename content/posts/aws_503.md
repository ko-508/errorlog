...
- **ターゲットグループの[ヘルスチェック](/glossary/ヘルスチェック/)失敗**：ヘルスチェックパス、[プロトコル](/glossary/プロトコル/)、[ポート](/glossary/ポート/)番号が正しいか確認。例えば、`/health` [エンドポイント](/glossary/エンドポイント/)が存在しない場合、すべてのターゲットが unhealthy になります。
- **セキュリティグループの設定**：ALB から[バックエンド](/glossary/バックエンド/)へのトラフィックが許可されているか確認。インバウンドルールでヘルスチェックポート（例：8080）を ALB のセキュリティグループから許可してください。

### API Gateway + Lambda の 503

[API](/glossary/api/) Gateway で 503 が返される主な原因：

- **Lambda 関数のエラー**：Lambda の[タイムアウト](/glossary/タイムアウト/)（デフォルト3秒）、メモリ不足、または実行[ロール](/glossary/ロール/)（[IAM](/glossary/iam/) Role）の権限不足により、関数が失敗します。CloudWatch [ログ](/glossary/ログ/)を確認してください。
...
- **[API](/glossary/api/) Gateway [スロットリング](/glossary/スロットリング/)**：ステージレベルのスロットルレート（デフォルト 10,000 RPS）を超えると 429 が返されます。必要に応じて上限引き上げを[リクエスト](/glossary/リクエスト/)してください。

### Amazon S3 の 503

S3 で 503 が返される場合、以下の対応を取ってください。

- **キーの先頭がランダムでない**：S3 は キープレフィックスで自動シャーディングされます。すべての[リクエスト](/glossary/リクエスト/)が同じプレフィックス（例：`uploads/2024-01-01-...`）に集中すると、503 が発生します。タイムスタンプをシャッフルするか、ハッシュプレフィックスを追加してください。
