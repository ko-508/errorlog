...
AWS や Google Cloud などのマルチリージョンサービスでは、リージョン指定を誤ると別リージョンのリソースを探してしまい、404 [エラー](/glossary/エラー/)が発生します。リソース ID やリソース名の入力間違いも同様に原因となります。
...
AWS の場合、リソース ID には各リージョン固有の値が使われるため、リージョン指定の誤りは一般的に 404 [エラー](/glossary/エラー/)に繋がります。Google Cloud でも同様なことが言えるため、複数リージョンで運用する場合は provider alias を活用し、リソース参照時に明示的に provider を指定するのがベストプラクティスです。
