...
Nginx における 400 エラーは、クライアントから送信された[リクエスト](/glossary/リクエスト/)が [HTTP](/glossary/http/) 仕様に違反していることを示します。リクエストヘッダーの形式が不正、サイズ超過、または URI の不正な文字エンコーディングなどが原因となり、サーバー側で処理できない状態を意味します。本エラーはクライアント側の問題であるため、サーバー設定と[リクエスト](/glossary/リクエスト/)内容の両面から原因特定が必要です。
