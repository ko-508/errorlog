...
Azure のデータセンター障害やメンテナンス作業により、特定のリージョン全体がサービス停止している場合があります。この場合、[アプリケーション](/glossary/アプリケーション/)側での[修正](/glossary/修正/)では解決できず、Azure のサービス復旧を待つか、別リージョンへの切り替えが必要です。
...
トラフィック急増時に、App Service や AKS の[インスタンス](/glossary/インスタンス/)数が[自動スケーリング](/glossary/自動スケーリング/)の速度に追いつかず、処理可能な[リクエスト](/glossary/リクエスト/)数を超える場合があります。Auto Scaling ルールが不適切に[設定](/glossary/設定/)されていると、スケールアウトが[遅延](/glossary/遅延/)し、[リクエスト](/glossary/リクエスト/)が処理されずに 503 を返します。
...
バックエンドプールの正常性プローブが失敗している、またはバックエンドインスタンスの[ポート](/glossary/ポート/)[設定](/glossary/設定/)が間違っている場合、Application Gateway が[リクエスト](/glossary/リクエスト/)をルーティングできず 503 を返します。
...
Free プランや Shared プランでは Auto Scaling が利用できないため、ピーク時のトラフィック対応ができません。[本番環境](/glossary/本番環境/)では最低でも Standard プラン以上を使用し、事前に負荷[テスト](/glossary/テスト/)を実施して[インスタンス](/glossary/インスタンス/)数を決定してください。

### リトライポリシーの実装

[クライアント](/glossary/クライアント/)側では、503 [エラー](/glossary/エラー/)を受け取った場合の指数バックオフリトライポリシーを実装することが重要です。
...
- **[CPU](/glossary/cpu/) 使用率**：90% 以上が継続している場合、スケーリング[設定](/glossary/設定/)を見直してください
- **[メモリ](/glossary/メモリ/)使用率**：予期しないメモリリークがないか確認します
