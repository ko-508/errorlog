...
**`error` [オブジェクト](/glossary/オブジェクト/)ではなく `detail` 配列**です。項目の名前もまったく違います。これは、多くの互換[サーバー](/glossary/サーバー/)が特定のフレームワークの上に作られており、その検証機構がこの形式で[エラー](/glossary/エラー/)を返すためです。末尾に検証ライブラリの説明への [URL](/glossary/url/) が入る点も特徴的です。
...
まず接続先です。要求先は `http://localhost:5001/v1/embeddings`、つまり**手元で動かしている推論[サーバー](/glossary/サーバー/)**でした。次に応答本文です。`error` [オブジェクト](/glossary/オブジェクト/)ではなく `detail` 配列で、各要素に `type`、`loc`、`msg`、そして検証ライブラリの説明ページへの [URL](/glossary/url/) が入っています。
