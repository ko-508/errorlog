...
NoSuchKey [エラー](/glossary/エラー/)は **[IAM](/glossary/iam/)権限不足でも発生** することがあります。例えば、`s3:GetObject` [権限](/glossary/権限/)がユーザーに付与されていない場合、[オブジェクト](/glossary/オブジェクト/)が存在していても同じ[エラーメッセージ](/glossary/エラーメッセージ/)が返されることがあります。権限関連が疑われる場合は、[オブジェクト](/glossary/オブジェクト/)存在確認と並行して [IAM](/glossary/iam/) [ポリシー](/glossary/ポリシー/)のレビューを行ってください。
...
- **Cloudflare R2** ：S3互換の[API](/glossary/api/)を提供しながら、デフォルトで出力（egress）料金が無料という特性があり、小〜中規模のアプリケーションで NoSuchKey による頻繁な再試行を許容しやすい運用環境を構築できます。

## Editor's Note

AWS 公式ドキュメントではNoSuchKey [エラー](/glossary/エラー/)を「[オブジェクト](/glossary/オブジェクト/)が存在しない」という単純な説明で扱っていることが多いため、実装者は直感的にキー存在性のみに原因を絞りがちです。しかし、[Stack Overflow での報告](https://stackoverflow.com/questions/44778448/s3-giving-me-nosuchkey-error-even-when-the-key-exists)では、「キーは確実に存在しているのに NoSuchKey が出る」というケースが頻繁に報告されており、これは大文字小文字の区別やパス区切り文字、[IAM](/glossary/iam/)[アクセス権限](/glossary/アクセス権限/)の問題が複合的に絡んでいることを示唆しています。また、LocalStack（S3をローカル環境で模擬するツール）の [GitHub Issue #8174](https://github.com/localstack/localstack/issues/8174) では、[バージョニング](/glossary/バージョニング/)有効時の削除マーカー処理が公式S3と異なる動作をする問題が報告されており、[テスト](/glossary/テスト/)環境と本番環境で[エラー](/glossary/エラー/)の再現性が異なるパターンも存在します。現場では、HeadObject での事前確認とCloudTrail[ログ](/glossary/ログ/)の並行確認から着手するのが有効です。
