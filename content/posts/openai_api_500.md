...
> この記事では、**OpenAI [API](/glossary/api/)** を使っているときに表示される **500** というエラーの意味と、その直し方を順を追って説明します。
...
OpenAI [API](/glossary/api/) で 500 が出るときに、最もよく見られる原因を挙げます。
自分の状況に当てはまるものを探してみてください。

- OpenAIの[インフラ](/glossary/インフラ/)で内部的な障害が起きている
- 特定の[リクエスト](/glossary/リクエスト/)内容がサーバーエラーを引き起こしている（まれに）
...
1. 数秒後に再試行する（指数[バックオフ](/glossary/バックオフ/)を推奨）
1. 問題が継続する場合はOpenAIサ[ポート](/glossary/ポート/)に[リクエスト](/glossary/リクエスト/)内容と[エラーレスポンス](/glossary/エラーレスポンス/)を添えて報告する

---

## まとめ

OpenAI [API](/glossary/api/) の **500** エラーは、上記のいずれかの原因によって発生するケースがほとんどです。
