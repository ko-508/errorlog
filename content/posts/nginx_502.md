...
Nginx 設定ファイル内の `upstream` ブロックで指定されたアドレスや[ポート](/glossary/ポート/)が実際のバックエンドサーバーと異なる場合、接続に失敗します。
...
複数のバックエンドサーバーを設定している場合、全てのサーバーが応答不可になると 502 が発生します。[ヘルスチェック](/glossary/ヘルスチェック/)機能を活用して、障害サーバーを自動的に除外することができます：
