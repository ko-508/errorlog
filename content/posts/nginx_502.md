...
error_type: "502"
components: []
related_services: ["Gunicorn", "Node.js", "Apache"]
---

## エラーの概要

502 Bad Gateway [エラー](/glossary/エラー/)は、Nginx がリバースプロキシとしてバックエンドサーバー（アプリケーションサーバーやアップストリームサーバー）から有効な[レスポンス](/glossary/レスポンス/)を受け取れないときに発生します。クライアント側の[リクエスト](/glossary/リクエスト/)は正常に到達していますが、Nginx が[バックエンド](/glossary/バックエンド/)との[通信](/glossary/通信/)に失敗した状態です。この[エラー](/glossary/エラー/)が出ているということは、Nginx 自体は稼働していますが、その背後のアプリケーションレイヤーに問題があることを示唆しています。
...
[エラーログ](/glossary/エラーログ/)（`/var/log/nginx/error.log`）の出力例：
...
アプリケーションサーバーが重い処理を実行中で応答が遅い場合、Nginx のデフォルトタイムアウト（通常 60 秒）に間に合わず[エラー](/glossary/エラー/)になります。または、[バックエンド](/glossary/バックエンド/)が一時的にハング状態に陥っているケースもあります。
...
[バックエンド](/glossary/バックエンド/)からのレスポンスサイズが大きい場合、Nginx がバッファにしきい値を超えるデータを受け取ると 502 [エラー](/glossary/エラー/)になることがあります。以下で対応します：
...
[バックエンド](/glossary/バックエンド/)が [HTTPS](/glossary/https/) の場合、ホスト名検証[エラー](/glossary/エラー/)で 502 が発生することがあります：
...
Nginx [エラーログ](/glossary/エラーログ/)を詳細に確認します：
