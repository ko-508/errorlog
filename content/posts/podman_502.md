...
Podman の 502 [エラー](/glossary/エラー/)は、公式ドキュメントではコンテナレジストリの [SSL](/glossary/ssl/) 証明書[エラー](/glossary/エラー/)や [DNS](/glossary/dns/) 解決失敗が主要な原因として説明されていますが、実際の報告からは[ネットワーク設定の不備](https://www.reddit.com/r/podman/comments/16g48w9/podman_reverse_proxy_returning_error_502_connection/)と[ランタイム変更による互換性問題](https://www.reddit.com/r/archlinux/comments/1cjm063/podman_krun_502_bad_gateway/)が同程度に発生していることが分かります。特に Podman v5.0 への[アップグレード後のネットワークスタック変更](https://github.com/containers/podman/issues/27308)に関しては、公式ドキュメントで詳細な移行ガイドが整備されていません。現場では「[レジストリ](/glossary/レジストリ/)の [SSL](/glossary/ssl/) 証明書を確認する」よりも先に「リバースプロキシ設定で `host.containers.internal` を使用しているか」を確認してから、ランタイム・ネットワークスタックを順に検証するのが有効です。

> **調査について**　この記事の解決策は、Reddit・GitHub Issues への公開報告（Podman v4.x, Podman v5.0 環境での報告を含む）を Gemini + Google Search で検索・精査し、実効性の高いものを整理したものです。参照元の URL は Editor's Note に記載しています。
