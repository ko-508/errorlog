...
- [CI/CD](/glossary/ci-cd/)パイプラインが短時間に大量の pull を実行している
...
1. [Docker](/glossary/docker/) Hub にログインすることで取得上限を引き上げる
2. よく使うイメージを[プライベートレジストリー](/glossary/プライベートレジストリー/)またはローカルキャッシュに保存する
3. pull の頻度を下げるよう CI 設定を見直す

それでも解決しない場合は、[Docker](/glossary/docker/) の公式ドキュメントで最新の情報を確認するか、公式のコミュニティフォーラムやサポートに問い合わせてください。
