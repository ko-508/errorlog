...
更新版が無く、実際には動くと確認できている場合に限り、根本の[設定ファイル](/glossary/設定ファイル/)で置き換えを指定できます。公式ドキュメントは、この指定が根本の[ファイル](/glossary/ファイル/)でのみ有効で、導入済みのパッケージ側の指定は解決に使われないと明記しています。
...
`E404` は、その名前のパッケージ自体が見つからない場合です。名前の誤りか、私用の置き場への[認証](/glossary/認証/)が通っていない場合に出ます。
