...
OpenAI [API](/glossary/api/) の 409 Conflict [エラー](/glossary/エラー/)は、[送信](/glossary/送信/)した[リクエスト](/glossary/リクエスト/)が既に進行中の操作や[サーバー](/glossary/サーバー/)上のリソースの現在の状態と競合しています。Fine-tuningジョブの重複実行、ファイルアップロード中の再実行、Batch [API](/glossary/api/) の重複送信など、同時実行不可の操作を試みた際に発生します。
