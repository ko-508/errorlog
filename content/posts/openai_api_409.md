...
同じトレーニングデータや[モデル](/glossary/モデル/)に対して、既に実行中のFine-tuningジョブがあるのに、さらに同じ設定で新しいジョブを作成しようとするとこの[エラー](/glossary/エラー/)が発生します。OpenAI [API](/glossary/api/) はアカウント内で同時に実行できるFine-tuningジョブの数に制限があり、また同一のトレーニングデータセットに対して並行実行できないためです。

**ファイルのアップロード競合**

Files [API](/glossary/api/) を使用してトレーニングファイルをアップロードする際に、既にアップロード中や処理中のファイルIDに対して再度アップロードリクエストを送信すると競合が発生します。同じファイルIDに複数のアップロード操作が並行実行されようとするとき、この409[エラー](/glossary/エラー/)が返されます。
