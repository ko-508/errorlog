...
2つ目はクォータの不足です。`type` は `insufficient_quota`、文言は現在のクォータを超過したので契約と請求の[設定](/glossary/設定/)を確認せよ、という趣旨になります。こちらは**待っても永久に直りません**。原因が[送信](/glossary/送信/)の速さではなく、残高や請求の状態にあるためです。
...
**Before（再試行の[設定](/glossary/設定/)を強める）：**
...
支出の上限と[レート制限](/glossary/レート制限/)も別物です。公式には、組織ごとに承認された月間の利用上限があり、これは[設定](/glossary/設定/)できる支出上限とは別だと書かれています。
