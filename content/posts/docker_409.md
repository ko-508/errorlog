...
date: 2026-01-01
description: "Docker の 409 エラーの原因と解決策。リクエストの内容がサーバーの現在の状態と矛盾している。対処手順をわかりやすく解説します。"
tags: ["Docker"]
errorCode: "409"
---

> この記事では、**Docker** を使っているときに表示される **409** というエラーの意味と、その直し方を順を追って説明します。
...
- 同じ名前のコンテナがすでに動いているか停止状態で残っている
- 同じポートを複数のコンテナが使おうとしている
...
1. docker ps -a で同名コンテナを確認し docker rm コンテナ名 で削除する
1. docker run 時に --name を変更するか既存コンテナを先に削除する
...
Docker の **409** エラーは、上記のいずれかの原因によって発生するケースがほとんどです。
