...
Slack [API](/glossary/api/)ではチャンネル識別に「チャンネルID」（C で始まる文字列、例：C123456789）を使用します。一方、UI上の「#channel-name」はチャンネル名です。[API](/glossary/api/)[リクエスト](/glossary/リクエスト/)にチャンネル名を渡すと、たとえチャンネルが存在していても403[エラー](/glossary/エラー/)になることがあります。
