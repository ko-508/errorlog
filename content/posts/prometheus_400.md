...
[HTTP](/glossary/http/) [API](/glossary/api/) 使用時は、ユーザー入力をクエリパラメータに含める場合、[URL](/glossary/url/) エンコード処理が正しく行われているか確認も必要です。特にプログラミング言語の [HTTP](/glossary/http/) ライブラリを使う際、手動で [URL](/glossary/url/) 文字列を組み立てるとエンコード漏れが発生しやすくなります。
