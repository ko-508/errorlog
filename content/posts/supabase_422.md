...
**パスワードポリシーの設定：**
Supabase Dashboardの「Authentication」→「Policies」で、[パスワード](/glossary/パスワード/)の最小文字数、複雑性要件、有効期限などをカスタマイズできます。デフォルトより厳しい設定にしている場合は、そのルールに合わせたバリデーションをフロントエンドに実装してください。
