...
Vercel の 503 エラーは、Vercel のサーバーが一時的に要求を処理できない状態を示します。このエラーは、Vercel のインフラストラクチャー側の問題またはデプロイされたアプリケーションのリソース制限に起因することが多く、ユーザーのアクセスが一時的にブロックされます。通常は数分から数時間で自動的に復旧しますが、原因の特定と対応は重要です。
...
ブラウザでアクセスすると、Vercel の標準エラーページが表示されるか、上記のようなレスポンスが返却されます。
...
エッジミドルウェアは認証やリダイレクトなど軽量な処理に向いており、同時実行数制限がより高く設定されています。長時間の処理はサーバーレス関数で実行し、キューイングシステムを組み合わせるべきです。
