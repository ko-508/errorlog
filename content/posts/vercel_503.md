...
Vercel の 503 [エラー](/glossary/エラー/)は、Vercel の[サーバー](/glossary/サーバー/)が一時的に[リクエスト](/glossary/リクエスト/)を処理できない状態を示します。このエラーは、Vercel のインフラストラクチャー側の問題または[デプロイ](/glossary/デプロイ/)されたアプリケーションのリソース制限に起因することが多く、ユーザーのアクセスが一時的にブロックされます。通常は数分から数時間で自動的に復旧しますが、原因の特定と対応は重要です。
...
Vercel のグローバルネットワークやデータセンター側で一時的なサービス中断が発生している場合、全ユーザーの[デプロイ](/glossary/デプロイ/)が 503 エラーを返します。これはユーザーのコード変更とは関係なく発生します。
...
新規デプロイ後、アプリケーションがまだ完全に初期化されていない状態で大量のトラフィックが流入すると、複数の関数[インスタンス](/glossary/インスタンス/)が同時に起動しようとして 503 が発生することがあります。
...
エッジミドルウェア（[ネットワーク](/glossary/ネットワーク/)の周辺で実行される軽量な処理層）は[認証](/glossary/認証/)やリダイレクトなど軽量な処理に向いており、同時実行数制限がより高く設定されています。長時間の処理はサーバーレス関数で実行し、キューイングシステムを組み合わせるべきです。
...
キャッシュとレート制限（同一ユーザーのリクエスト数を制限する機能）を組み合わせることで、サーバーレス関数の過負荷を防げます。
