...
Vercel のグローバルネットワークやデータセンター側で一時的なサービス中断が発生している場合、全ユーザーの[デプロイ](/glossary/デプロイ/)が 503 [エラー](/glossary/エラー/)を返します。これはユーザーのコード変更とは関係なく発生します。
...
Vercel の[サーバーレス](/glossary/サーバーレス/)関数（Next.js [API](/glossary/api/) Routes、Edge Functions など）には、プランごとに同時実行数の上限が設定されています。大量のアクセスや長時間の処理を実行する関数が複数起動すると、すぐに上限に達し、新規[リクエスト](/glossary/リクエスト/)が 503 で拒否されます。
...
新規[デプロイ](/glossary/デプロイ/)後、[アプリケーション](/glossary/アプリケーション/)がまだ完全に[初期化](/glossary/初期化/)されていない状態で大量のトラフィックが流入すると、複数の関数[インスタンス](/glossary/インスタンス/)が同時に起動しようとして 503 が発生することがあります。
...
エッジミドルウェア（[ネットワーク](/glossary/ネットワーク/)の周辺で実行される軽量な処理層）は[認証](/glossary/認証/)やリダイレクトなど軽量な処理に向いており、同時実行数制限がより高く設定されています。長時間の処理は[サーバーレス](/glossary/サーバーレス/)関数で実行し、キューイングシステムを組み合わせるべきです。
...
[キャッシュ](/glossary/キャッシュ/)と[レート制限](/glossary/レート制限/)（同一ユーザーの[リクエスト](/glossary/リクエスト/)数を制限する機能）を組み合わせることで、[サーバーレス](/glossary/サーバーレス/)関数の過負荷を防げます。
