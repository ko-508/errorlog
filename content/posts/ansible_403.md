...
Ansibleで403エラーが発生する場合、ターゲットシステムへのアクセス権限が不足していることを意味します。このエラーはSSH接続後、実行対象のタスクやファイル操作時に権限不足を検出した際に表示されます。Ansibleが接続したユーザーアカウントに必要な権限がないため、コマンド実行やファイル読み書きが拒否される状況です。
...
Ansibleで接続したユーザーアカウントに対して、sudo実行権限そのものが付与されていないケースです。`become: true`を指定してroot権限での実行を試みても、sudo権限がなければタスクは失敗します。
...
この場合、接続ユーザー（例：ubuntu）がsudo権限を持たないと403エラーが発生します。
...
sudoersファイルで特定のコマンドのみを許可している場合、Ansibleが実行しようとするコマンドが許可リストに含まれていないと403エラーが発生します。
...
上記の場合、`systemctl restart networking`は許可リストに含まれないため403エラーが発生します。
...
ターゲットホスト上のファイルやディレクトリに対して、接続ユーザーが読み取り・書き込み権限を持たないケースです。sudo権限があっても、特定のファイル操作が明示的に拒否されることがあります。
...
Ansibleの403エラーは権限不足を示していますが、実際の原因特定にはいくつかのポイントがあります。

**ターゲットホストでの事前確認**

接続予定のユーザーで以下を実行し、実際のsudo権限を確認してください。

```bash
sudo -l
```

このコマンドの出力から、そのユーザーが実行可能なコマンドが明記されます。`(ALL) NOPASSWD:ALL`と表示されれば、すべてのコマンドが許可されている状態です。
...
SELinuxが有効な環境では、ファイルアクセス権限が追加で制限されます。403エラーが表示される場合、SELinuxコンテキストの確認も必要です。
...
権限エラーの詳細はターゲットホストのsyslogに記録されていることがあります。
