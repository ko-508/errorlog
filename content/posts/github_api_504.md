...
第一に、その[コード](/glossary/コード/)が本当に GitHub から返っているかを確認します。手元の[クライアント](/glossary/クライアント/)の[タイムアウト](/glossary/タイムアウト/)（curl の --max-time、requests の timeout= など）が先に発火した場合、[ステータスコード](/glossary/ステータスコード/)は受け取れず、例外や接続打ち切りとして現れます。この場合の調査対象は GitHub ではなく、手元の設定と経路です。
...
504と混同されやすい事象の正しい行き先です。手元の[クライアント](/glossary/クライアント/)の[タイムアウト](/glossary/タイムアウト/)発火は、[コード](/glossary/コード/)が返らずに例外や接続打ち切りとして現れるもので、GitHub の504ではありません（調査対象は手元の設定です）。[トークン](/glossary/トークン/)の誤り・失効は 401 です（[401 の記事](/posts/github_api_401/)）。並列や高頻度の大量[リクエスト](/glossary/リクエスト/)が弾かれる場合、GitHub の仕様では 403 または 429 の secondary rate limit として現れ、待ち時間の指示に従うのが公式の対処です（[403 の記事](/posts/github_api_403/)、[429 の記事](/posts/github_api_429/)）。「大量に送ったから504になった」という説明は、混雑による時間切れ（原因1の変種）でない限り仕様に合いません。private リソースへの認証不備は、権限不足を隠すための 404 です（[404 の記事](/posts/github_api_404/)）。GitHub 内部の予期しない[エラー](/glossary/エラー/)で、時間切れとは限らないものは 500 です（[500 の記事](/posts/github_api_500/)）。[GraphQL](/glossary/graphql/) の重い[クエリ](/glossary/クエリ/)の時間切れは 502 の形の報告が多く、分割の考え方は共通です（[502 の記事](/posts/github_api_502/)）。

## 切り分けの順序

1. [コード](/glossary/コード/)が GitHub から返っているかを確認する。手元の[タイムアウト](/glossary/タイムアウト/)の発火なら、手元の設定と経路の調査に切り替える。
...
504は「GitHub が時間切れで打ち切った」という一点だけを伝える[コード](/glossary/コード/)です。手元の[タイムアウト](/glossary/タイムアウト/)との区別、稼働状況、そして要求の重さ。この3つを順に確認すれば、[コード](/glossary/コード/)をいじるべきか、待つべきか、分割すべきかが決まります。
