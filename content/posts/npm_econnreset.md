...
`npm error code ECONNRESET` は、接続が相手側から切られたという意味です。npm はこの[コード](/glossary/コード/)を他の[ネットワーク](/glossary/ネットワーク/)系と同じ分岐で扱うため、固有の説明は出ません。表示されるのは「[ネットワーク](/glossary/ネットワーク/)接続に関する問題である」「多くの場合は中継の[設定](/glossary/設定/)か[ネットワーク](/glossary/ネットワーク/)[設定](/glossary/設定/)に問題がある」という2文と、中継設定の確認を促す1文だけです。
...
まず、経路の[設定](/glossary/設定/)を並べて確認します。
...
`proxy` と `https-proxy` が `null` なのに社内の[回線](/glossary/回線/)から実行している場合、原因1に当たります。環境変数側だけに[設定](/glossary/設定/)されていることもあるため、そちらも見てください。
...
対象が実行ごとに変わるなら、特定の[パッケージ](/glossary/パッケージ/)ではなく接続の総量が問題です。同じ対象で止まるなら、その向き先が届いていません。
...
確認方法は[設定値](/glossary/設定値/)の突き合わせです。公式の説明によれば、`HTTP_PROXY` や `http_proxy` の[環境変数](/glossary/環境変数/)が[設定](/glossary/設定/)されていれば、その内容が利用されます。npm 側の[設定](/glossary/設定/)と[環境変数](/glossary/環境変数/)のどちらか一方だけに値が入っていると、経路が定まりません。
...
中継先を通さない宛先がある場合は、除外する一覧も[設定](/glossary/設定/)してください。
...
改善するなら確定です。恒久的に[設定](/glossary/設定/)する場合は、実行環境ごとの[設定ファイル](/glossary/設定ファイル/)へ書いてください。
...
`strict-ssl` を無効にすると通ることがありますが、これは検証そのものを止める操作です。公式の[設定](/glossary/設定/)では既定で有効になっています。無効化は、なりすましを検知できなくなる点を理解したうえで、一時的な切り分けに限って使ってください。
