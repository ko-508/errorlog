...
> この記事では、**[Docker](/glossary/docker/)** を使っているときに表示される **500** というエラーの意味と、その直し方を順を追って説明します。
...
> [Docker](/glossary/docker/)[デーモン](/glossary/デーモン/)（バックグラウンドプロセス）で予期しない問題が起きた。
...
[Docker](/glossary/docker/) で 500 が出るときに、最もよく見られる原因を挙げます。
自分の状況に当てはまるものを探してみてください。

- [Docker](/glossary/docker/)[デーモン](/glossary/デーモン/)自体がクラッシュまたは異常な状態になっている
- ディスクの空き容量がなくなっている
...
1. sudo systemctl restart docker または [Docker](/glossary/docker/) Desktopを再起動する
1. df -h でディスク容量を確認し、docker system pruneで不要データを削除する
1. [Docker](/glossary/docker/)のログ（/var/log/docker.log）でエラーの詳細を確認する

---

## まとめ

[Docker](/glossary/docker/) の **500** エラーは、上記のいずれかの原因によって発生するケースがほとんどです。
