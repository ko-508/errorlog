...
つまりこの[エラー](/glossary/エラー/)は、値の書き方を咎めているのではありません。インスタンスの名前として使えない値が渡された、と言っています。直す先は式ではなく、何をキーにするかという設計です。
...
別のリソースが作られてはじめて分かる[属性](/glossary/属性/)を、キーの側に使っている場合です。公式ドキュメントは、`for_each` が反復するすべての値が遠隔操作の前に確定していなければならず、作成後にしか分からない一意な識別番号などを参照するとエラーになる、と明記しています。
...
`Unsupported argument` は、引数の名前がその位置に存在しない場合の診断です。`for_each` の値ではなく、書いた引数名の側の問題です（[Terraform の Unsupported argument の記事](/posts/terraform_unsupported_argument/)）。
