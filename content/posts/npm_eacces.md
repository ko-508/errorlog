...
これで通るなら、失敗しているのは導入そのものではなく[スクリプト](/glossary/スクリプト/)です。対処は、その[パッケージ](/glossary/パッケージ/)が何をしようとしているかを確認したうえで判断することになります。書き込み先を[設定](/glossary/設定/)で変えられる場合が多くあります。

`--ignore-scripts` を恒久的な[設定](/glossary/設定/)にすると、正常に必要な[スクリプト](/glossary/スクリプト/)も動かなくなります。切り分けの手段として使ってください。
...
npm の文面を組み立てる処理は、`EACCES` と `EPERM` を同じ分岐で扱います。分岐の中で最初に判定するのが、失敗した経路が[キャッシュ](/glossary/キャッシュ/)配下かどうかです。[エラー](/glossary/エラー/)[オブジェクト](/glossary/オブジェクト/)の `path` と `dest` のいずれかが、[設定](/glossary/設定/)されている[キャッシュ](/glossary/キャッシュ/)の値で始まっているかを見ます。
...
[設定](/glossary/設定/)の既定値も押さえておきます。[キャッシュ](/glossary/キャッシュ/)の置き場は、公式の説明によれば Windows が `%LocalAppData%\npm-cache`、POSIX 系が `~/.npm` です。`prefix` は、全体向けの動作では node の実行[ファイル](/glossary/ファイル/)が置かれている[ディレクトリ](/glossary/ディレクトリ/)が既定になります。

## バージョン差・注意点

古い手順で見かける `--unsafe-perm` は、現在の npm には存在しません。npm 6 の設定定義には確かに含まれており、Windows か cygwin か、あるいは実行者が root でない場合に真になる作りでした。しかし npm 7.0.0 の設定定義には既に含まれておらず、8 系にも最新版にもありません。`sudo npm install -g --unsafe-perm` という指示を見かけても、現在の npm では[設定](/glossary/設定/)として認識されません。
...
当時の状態としては、npm 6 系が広く使われており、root で実行したときに[スクリプト](/glossary/スクリプト/)の実行[権限](/glossary/権限/)を落とす挙動があったため、それを無効にする[設定](/glossary/設定/)として `--unsafe-perm` が使われていました。実装を見ると、Windows か cygwin の場合、利用者番号を扱う機能が使えない場合、あるいは実行者が root でない場合に既定で真になる形でした。裏返せば、root で実行したときだけ既定で偽になり、そこで問題が起きていたわけです。
