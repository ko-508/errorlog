...
`path` が[キャッシュ](/glossary/キャッシュ/)でも `prefix` でも作業[ディレクトリ](/glossary/ディレクトリ/)でもない場合です。パッケージの導入後[スクリプト](/glossary/スクリプト/)が、システムの領域や他の利用者の領域に書こうとしています。
...
これで通るなら、失敗しているのは導入そのものではなく[スクリプト](/glossary/スクリプト/)です。対処は、そのパッケージが何をしようとしているかを確認したうえで判断することになります。書き込み先を設定で変えられる場合が多くあります。
