...
Vercel API を直接呼び出す場合、認証トークンやプロジェクト ID、デプロイ対象のファイル情報など、必須パラメーターの不足が 400 エラーを引き起こします。特にカスタムスクリプトや CI/CD パイプラインから自動デプロイする場合、リクエストボディの構造を厳密に確認する必要があります。
...
環境変数の値の型も厳密です。数値は文字列で囲む必要があり、配列やオブジェクトは正しくシリアライズされていなければなりません。特に API を直接呼び出す場合、ファイルコンテンツは Base64 エンコード（64進法の文字列形式に変換）されたテキストとして送信する必要があり、バイナリのまま送信すると 400 エラーが発生します。
