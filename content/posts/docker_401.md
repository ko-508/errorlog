...
> この記事では、**[Docker](/glossary/docker/)** を使っているときに表示される **401** というエラーの意味と、その直し方を順を追って説明します。
...
> [認証](/glossary/認証/)情報が提供されていないか、[認証](/glossary/認証/)に失敗した。
...
- [認証](/glossary/認証/)[トークン](/glossary/トークン/)の有効期限が切れている
- [レジストリ](/glossary/レジストリ/)のURLが間違っている
...
1. docker loginコマンドで再度ログインする
1. ~/.docker/config.jsonを削除してdocker loginをやり直す
1. 接続先の[レジストリ](/glossary/レジストリ/)URLを公式ドキュメントで確認する

---

## まとめ

[Docker](/glossary/docker/) の **401** エラーは、上記のいずれかの原因によって発生するケースがほとんどです。
