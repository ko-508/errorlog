...
もう1つの落とし穴が共有メモリゾーンの枯渇です。ゾーンの空きがなくなると最も古い状態から削除されますが、それでも新しい状態を作れない場合、その[リクエスト](/glossary/リクエスト/)は拒否されます。公式ドキュメントによると、状態1件は64[ビット](/glossary/ビット/)環境で128[バイト](/glossary/バイト/)を占め、1MB のゾーンに約8千件です。[クライアント](/glossary/クライアント/)数に対してゾーンが小さすぎないか確認してください。
...
`proxy_pass` 構成では、上流の応答コードは既定でそのまま[クライアント](/glossary/クライアント/)に中継されます（`proxy_intercept_errors` の既定は off）。上流の[アプリケーション](/glossary/アプリケーション/)がメンテナンスモードだったり、過負荷への保護機能で自ら 503 を返していたりすれば、Nginx を経由した応答も 503 になります。
