...
**503** は、Nginx が「Nginxの[バックエンド](/glossary/バックエンド/)が過負荷またはメンテナンス中で[リクエスト](/glossary/リクエスト/)を処理できない。」という状態のときに表示されます。
...
- upstream内のすべての[バックエンド](/glossary/バックエンド/)サーバーがダウンしている
- [バックエンド](/glossary/バックエンド/)の最大接続数（max_conns）に達している
...
1. upstream内の各[バックエンド](/glossary/バックエンド/)サーバーの状態を確認する
1. max_conns値を増やすか[バックエンド](/glossary/バックエンド/)サーバーをスケールアップする
