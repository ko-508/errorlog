...
この3つは、対処が根本的に違います。定義の説明文が、その違いをはっきり述べています。1つ目は「系の状態に関係なく問題のある[引数](/glossary/引数/)」を指し、例として形式の壊れた[ファイル](/glossary/ファイル/)名が挙げられています。何度送っても結果は変わりません。2つ目は「系がその操作に必要な状態にない」ことを指し、例として空でない[ディレクトリ](/glossary/ディレクトリ/)の削除が挙げられています。状態を直せば同じ要求が通ります。3つ目は「有効な範囲を越えた操作」で、[ファイル](/glossary/ファイル/)の終端を越えて読もうとした場合が例です。
...
送った値に誤りはありません。対象が今その操作を受け付けられる状態にないだけです。定義の説明にある空でない[ディレクトリ](/glossary/ディレクトリ/)の削除が、そのまま典型例です。
...
名札の貼り替えが実際に起きている記録もあります。開発支援の道具から Gemini の窓口を呼んだ際の報告（[Aider-AI/aider Issue #3817](https://github.com/Aider-AI/aider/issues/3817)）では、返ってきた応答そのものは `status` が `INVALID_ARGUMENT`、`ErrorInfo` の `reason` が `API_KEY_INVALID` の 400 でした。鍵という[引数](/glossary/引数/)が、それ自体として不正だという通知です。ところが間に挟まった部品はこれを[認証](/glossary/認証/)の[エラー](/glossary/エラー/)として包み直し、提供元があなたを[認証](/glossary/認証/)できない、という文言で表示しています。この表示から 401 を想定して[認証](/glossary/認証/)の仕組みを調べ始めれば、遠回りです。応答の `status` と `reason` を直接読む、という本記事の順序は、途中の道具が貼り替えた名札に惑わされないための順序でもあります。
...
=======
*免責事項：本記事の内容は、執筆時点の公開情報をもとに作成したものです。[ソフトウェア](/glossary/ソフトウェア/)の仕様は予告なく変更されることがあります。最新の情報は各[ツール](/glossary/ツール/)の公式サポートページをご確認ください。本記事の情報を利用した結果生じたいかなる損害についても、著者および運営者は責任を負いかねます。
>>>>>>> 9037bc9 (rewrite: gcp_400 記事を新しい質の型で書き直し)
