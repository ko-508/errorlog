...
**Realtime 機能との相性：** Realtime リスナーを有効にしているテーブルで競合が発生した場合、INSERT/UPDATE がロールバックされたことをリアルタイムで検知できます。クライアント側で[エラーハンドリング](/glossary/エラーハンドリング/)とリトライロジックを組み込むことを推奨します。
...
この[コマンド](/glossary/コマンド/)で `supabase/migrations/` ディレクトリに [SQL](/glossary/sql/) スキーマが出力され、実際の制約定義を目視確認できます。
