...
date: 2026-05-28
description: "指定したGoogle Cloudリソースが見つからない。。リソース名またはIDの綴りが間違っているなど、GCP 404 エラーの原因と解決策を解説。"
tags: ["GCP"]
errorCode: "404"
---
...
GCP のリソースID（[インスタンス](/glossary/インスタンス/)名、[バケット](/glossary/バケット/)名、[サービスアカウント](/glossary/サービスアカウント/)名など）は大文字小文字を区別します。また、ハイフンとアンダースコアの混同、数字の誤入力もよくある原因です。[API](/glossary/api/) [リクエスト](/glossary/リクエスト/)やコマンド実行時に誤ったリソース名を指定すると、システムが該当するリソースを見つけられず 404 エラーが返されます。
...
リソースが確実に別のプロジェクトに存在する場合、そのプロジェクトに対して適切な [IAM](/glossary/iam/) 権限があるかを確認します。Cloud Console で「[IAM](/glossary/iam/) と管理」→「[IAM](/glossary/iam/)」から、あなたのアカウントに必要な[ロール](/glossary/ロール/)（Viewer、Editor など）が割り当てられているか確認してください。それでも解決しない場合、GCP サ[ポート](/glossary/ポート/)に問い合わせる際は、エラーメッセージ、使用したコマンド、プロジェクトID、リソース名を含めて報告します。
