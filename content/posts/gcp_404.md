...
リソースが確実に別のプロジェクトに存在する場合、そのプロジェクトに対して適切な [IAM](/glossary/iam/) [権限](/glossary/権限/)があるかを確認します。Cloud Console で「[IAM](/glossary/iam/) と管理」→「[IAM](/glossary/iam/)」から、あなたのアカウントに必要な[ロール](/glossary/ロール/)（Viewer、Editor など）が割り当てられているか確認してください。それでも解決しない場合、GCP サポートに問い合わせる際は、エラーメッセージ、使用したコマンド、プロジェクトID、リソース名を含めて報告します。
