...
Jenkinsのジョブで実行するシェルスクリプトやGroovy Script が未処理の例外をスロー、あるいは予期しない終了状態を返す場合、500[エラー](/glossary/エラー/)が発生することがあります。特にスクリプトの実行中にファイルが見つからない、[権限](/glossary/権限/)がない、外部[コマンド](/glossary/コマンド/)が存在しないなどの状況です。
