...
Terraform で 500 エラーが発生した場合、Terraform Cloud またはクラウドプロバイダー（AWS、Azure、GCP など）の API で内部エラーが生じています。このエラーはサーバー側の問題であり、ユーザーの設定ミスではありません。500 エラーは通常、一時的な障害か、リソース作成時のプロバイダー側の予期しない状況を示しています。
...
```bash
# 実行コマンド
# terraform apply を実行後、Terraform Cloud の UI で run ID をコピー
# terraform show <run-id> でログを確認
# または UI の "View logs" ボタンから詳細エラーを確認
```
...
クラウドプロバイダーの API に対して、Terraform の実行アカウントが十分な権限を持たない、または API リクエストレート制限に達した場合、500 エラー（または 403/429 エラー）が返されます。特に大量のリソースをデプロイする場合に発生しやすいです。
...
2. **Workspace 変数の検証**：Terraform Cloud の workspace 設定で、環境変数（`TF_VAR_` で始まる変数）や sensitive 変数が正しく設定されているか確認します。不正な値が設定されている場合、プロバイダー側で 500 エラーが返されます。

3. **エージェント実行モード**：セルフホステッド Terraform Cloud Agent を使用している場合、エージェントのログを確認してください。エージェント側のメモリ不足やネットワーク接続エラーが原因の場合があります。

```bash
# セルフホステッド Agent のログ確認（Docker 実行時）
...
以上の手順を実施しても解決しない場合、以下の情報を用意して Terraform のサポートに問い合わせてください：

- Run ID（Terraform Cloud 使用時）
- terraform version と provider version
- エラーが発生した時刻（UTC）
- クラウドプロバイダーのリージョン
- 関連するリソース タイプ（aws_instance など）
