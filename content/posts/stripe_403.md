...
Stripe の 403 [エラー](/glossary/エラー/)は、[認証](/glossary/認証/)には成功したものの、その[リクエスト](/glossary/リクエスト/)に対する十分な[権限](/glossary/権限/)や許可がないことを示します。[API](/glossary/api/) キーが有効であっても、アクセス対象のリソースや実行しようとする操作に対して権限不足の状態です。本番環境のデータ保護や機能制限の都合上、Stripe 側で意図的にアクセスをブロックしているケースが大半を占めます。
...
**Before（権限不足）:**
```bash
# <your-restricted-test-key> には "charges:read" のみ付与
curl -X POST https://api.stripe.com/v1/charges \
  -H "Authorization: Bearer <your-restricted-test-key>" \
  -d amount=1000 \
  -d currency=jpy \
  -d source=tok_visa
...
**After（制限解除または代替手段）:**
[ダッシュボード](/glossary/ダッシュボード/) → Settings → Account status で制限状況を確認します。制限が解除されるまで待つか、制限の種類によっては Stripe サポートに問い合わせて解除を[リクエスト](/glossary/リクエスト/)します。国際決済などの制限は通常、[アカウント](/glossary/アカウント/)本人確認やビジネス情報の充実化により自動的に解除されます。
...
Stripe の GitHub [リポジトリ](/glossary/リポジトリ/)で同じ問題を報告しているユーザーがいないか確認できます。また、Stripe サポートは 24 時間対応なので、[アカウント](/glossary/アカウント/)関連の制限が原因と疑われる場合は直接問い合わせることをお勧めします。
