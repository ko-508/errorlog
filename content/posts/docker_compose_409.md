...
[Docker](/glossary/docker/) Compose は、リソース作成時に[プロジェクト](/glossary/プロジェクト/)名を自動的に接頭辞として付与します。デフォルトでは[ディレクトリ](/glossary/ディレクトリ/)名が[プロジェクト](/glossary/プロジェクト/)名として使用されるため、同じ `docker-compose.yml` を異なる[ディレクトリ](/glossary/ディレクトリ/)にコピーして実行すると、リソース名が異なるものになります。複数環境で同じ[プロジェクト](/glossary/プロジェクト/)名を使用する場合は、`-p` オプションで明示的に指定します。
