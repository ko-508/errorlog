...
[Docker](/glossary/docker/) Compose は、リソース作成時にプロジェクト名を自動的に接頭辞として付与します。デフォルトでは[ディレクトリ](/glossary/ディレクトリ/)名がプロジェクト名として使用されるため、同じ `docker-compose.yml` を異なる[ディレクトリ](/glossary/ディレクトリ/)にコピーして実行すると、リソース名が異なるものになります。複数環境で同じプロジェクト名を使用する場合は、`-p` オプションで明示的に指定します。
...
強制的に環境を[リセット](/glossary/リセット/)する必要がある場合は以下の手順を実行します。
