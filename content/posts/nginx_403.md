...
Nginx の 403 Forbidden エラーは、クライアント（ブラウザ）が特定のファイルやディレクトリへのアクセスを試みたとき、Nginx がそのリソースの存在は確認できるが、アクセス権限がないと判断したときに発生します。これは[認証](/glossary/認証/)の失敗（401）とは異なり、認証情報は正しいが権限がないという状態です。実務では、ファイルパーミッション、ディレクトリ設定、または Nginx の設定ルールによって引き起こされることがほとんどです。
