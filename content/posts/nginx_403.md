...
この違いは[エラーログ](/glossary/エラーログ/)で明確に区別できます。404 は[ログ](/glossary/ログ/)に `No such file or directory` と記録され、403 は `Permission denied` や `is forbidden` と記録されます。[アクセスログ](/glossary/アクセスログ/)は「403 が起きた」ことだけを示し、なぜ起きたかは[エラーログ](/glossary/エラーログ/)が示します。したがって、403 の調査は[設定ファイル](/glossary/設定ファイル/)をいじる前に、まず[エラーログ](/glossary/エラーログ/)を読むことから始めます。
...
最も多い原因です。Nginx のワーカープロセスは、配信する[ファイル](/glossary/ファイル/)に読み取り[権限](/glossary/権限/)が必要です。[エラーログ](/glossary/エラーログ/)には `(13: Permission denied)` が出ます。
...
[権限](/glossary/権限/)を、[ディレクトリ](/glossary/ディレクトリ/) 755・[ファイル](/glossary/ファイル/) 644 に揃え、所有者をワーカーユーザーにします。
...
[ファイル](/glossary/ファイル/)自体が 644 で正しくても、ルート(`/`)から対象[ファイル](/glossary/ファイル/)までの[パス](/glossary/パス/)上のどこかの[ディレクトリ](/glossary/ディレクトリ/)に、ワーカーユーザーの実行([検索](/glossary/検索/))[権限](/glossary/権限/)がないと、Nginx はそこを通り抜けられず 403 になります。見落としやすい典型例です。
...
[ディレクトリ](/glossary/ディレクトリ/)への[リクエスト](/glossary/リクエスト/)に対し、`index` ディレクティブで指定した[ファイル](/glossary/ファイル/)(既定では index.html)が存在せず、`autoindex` も off(既定)の場合、Nginx は 403 を返します。[エラーログ](/glossary/エラーログ/)には `directory index of "..." is forbidden` が出ます。

index [ファイル](/glossary/ファイル/)を置くか、[ディレクトリ](/glossary/ディレクトリ/)一覧を見せてよい場面なら autoindex を有効にします。
...
意図した制限ならそのままです。意図せず拒否している場合は、allow の範囲を見直すか、deny ルールを[修正](/glossary/修正/)します。アクセス元の IP が想定どおりか、[ログ](/glossary/ログ/)で確認してください。
...
Nginx 自体ではなく、`proxy_pass` や `fastcgi_pass` の先(PHP-FPM やアプリケーションサーバー)が 403 を返し、それがそのまま[クライアント](/glossary/クライアント/)に伝わるケースです。この場合、Nginx の[権限](/glossary/権限/)やコンテキストは正常で、原因は upstream 側にあります。
...
実際の報告例として、ユーザーのホームディレクトリ配下を Nginx で配信しようとして 403 になった事例があります([GitHub gist の議論](https://gist.github.com/jhjguxin/6208474))。なお、この議論は2013年頃の古いもので、当時の Ubuntu 12.04 を前提としています。ただし、ここで扱われている[パス](/glossary/パス/)[権限](/glossary/権限/)の考え方は、現在の Nginx でも変わりません。報告者の[環境](/glossary/環境/)では、ホームディレクトリの既定権限が制限的(700)で、Nginx のワーカーユーザーが親[ディレクトリ](/glossary/ディレクトリ/)を通り抜けられず 403 になっていました。本記事の原因2にあたります。この議論では、配信対象の[ファイル](/glossary/ファイル/)すべてに読み取り[権限](/glossary/権限/)を与えるだけでなく、ルートから対象までの[パス](/glossary/パス/)上のすべての親[ディレクトリ](/glossary/ディレクトリ/)に実行([検索](/glossary/検索/))[権限](/glossary/権限/)が必要だ、という点が解決の核心として挙げられています。[ディレクトリ](/glossary/ディレクトリ/) 755・[ファイル](/glossary/ファイル/) 644 に揃える対処が広く有効だと共有されています。

この事例が示すとおり、403 の調査では「[ファイル](/glossary/ファイル/)単体の[権限](/glossary/権限/)」だけでなく「[パス](/glossary/パス/)全体の通り抜け可否」を確認することが重要です。`namei -l` で[パス](/glossary/パス/)の各段を一度に点検するのが、確実な近道です。
