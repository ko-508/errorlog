...
date: 2026-07-29
description: "Pending は対象全体の段階の名前で、公式定義には機器に割り当てられる前の時間とイメージを取得している時間の両方を含むと書かれています。つまり性質の違う2つの状況を1語でまとめています。分岐は配置先が決まっているかどうかの1点で、そこから先の調べ方が完全に分かれます。"
tags: ["Kubernetes"]
errorCode: "Pending"
lastmod: 2026-07-29
service: "Kubernetes"
error_type: "Pending"
components: ["Scheduler", "Pod"]
related_services: ["kube-scheduler", "kubelet"]
trend_incident: false
---

## 冒頭まとめ

Pending は、[コンテナ](/glossary/コンテナ/)の状態を表す名前ではありません。対象全体がどの段階にあるかを表す名前です。公式の定義には、対象は受け付けられたが[コンテナ](/glossary/コンテナ/)の1つ以上がまだ起動していない状態であり、これには機器に割り当てられる前の時間と、[イメージ](/glossary/イメージ/)を取得している時間の両方が含まれる、と書かれています。
...
一覧では、準備できている[コンテナ](/glossary/コンテナ/)の数が0のまま止まります。
...
この例では、5台のうち3台が[メモリ](/glossary/メモリ/)の空き不足、2台が許容していない印によって除外されています。理由が2つ並んでいるので、対処も2通り考えられます。空きを増やすか、印を許容する[設定](/glossary/設定/)を加えるかです。

配置が済んでいる場合は、この記録が出ません。代わりに、[コンテナ](/glossary/コンテナ/)ごとの待機の理由が入ります。この場合、Pending という表示のまま[イメージ](/glossary/イメージ/)の取得や保管領域の準備が進んでいます。
...
第三に、起動準備の問題であれば、[コンテナ](/glossary/コンテナ/)ごとの待機の理由を見ます。
...
**Before（要求量が機器の[容量](/glossary/容量/)に対して大きすぎる）：**
...
印による除外であれば、許容する[設定](/glossary/設定/)を加えます。
...
待機中のままであれば、原因は2つ考えられます。条件に合う領域が用意されていない場合と、最初の利用者が決まるまで割り当てを保留する[設定](/glossary/設定/)になっている場合です。

後者の場合、配置と割り当てが互いを待つ状態にはなりません。配置が先に決まり、それから割り当てが行われます。したがって、この[設定](/glossary/設定/)で長く待機している場合は、配置のほうが別の理由で止まっています。原因1と原因2に戻ってください。

### 原因4：配置は済んでいるが起動準備で止まっている

配置先が入っているのに Pending のままの場合です。この段階では、[イメージ](/glossary/イメージ/)の取得、保管領域の接続、初期化用の[コンテナ](/glossary/コンテナ/)の実行などが行われています。

待機の理由を見れば、どこで止まっているかが分かります。取得に失敗している場合は専用の理由が入ります（[Kubernetes の ImagePullBackOff の記事](/posts/kubernetes_imagepullbackoff/)）。

初期化用の[コンテナ](/glossary/コンテナ/)が終わらない場合も、この段階に留まります。順番に実行され、すべて成功するまで本体は起動しません。
...
接続待ちの処理を初期化用の[コンテナ](/glossary/コンテナ/)に入れている場合、相手が起動しない限り永久に待ちます。待ち時間の上限を設けておくと、原因の特定が早くなります。
...
上限に達していれば、既存の対象を減らすか、上限を引き上げるかになります。なお、上限を設けている区画では、要求量を書いていない対象が受け付けられない[設定](/glossary/設定/)になっていることがあります。その場合、要求量を明記すれば通ります。

## 補足：似ているが別のもの

[コンテナ](/glossary/コンテナ/)が起動したあとに落ちている場合は、別の表示になります。一度は起動しているので、段階としては先へ進んでいます（[Kubernetes の CrashLoopBackOff の記事](/posts/kubernetes_crashloopbackoff/)）。

[メモリ](/glossary/メモリ/)の都合で止められている場合も、起動後の話です（[Kubernetes の OOMKilled の記事](/posts/kubernetes_oomkilled/)）。Pending の段階では、まだ[メモリ](/glossary/メモリ/)を使い始めていません。

配置が済んでいるのに機器の側が応答しない場合は、機器そのものの状態を確認してください。機器が正常でない状態にあると、そこへの新しい配置は行われません。

利用者向けの[通信](/glossary/通信/)への影響は、状態[コード](/glossary/コード/)として現れます。準備できている対象が1つも無い状態が続けば 503 になります（[Kubernetes の 503 の記事](/posts/kubernetes_503/)）。
...
6. 配置先が入っているなら、[コンテナ](/glossary/コンテナ/)ごとの待機の理由と、初期化用の[コンテナ](/glossary/コンテナ/)の進み具合を見る。
