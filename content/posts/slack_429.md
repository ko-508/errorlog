...
Slack Bolt（Python / JavaScript / Java）を使用している場合、フレームワークレベルで簡易的なレート制限対応が行われますが、完全に[自動化](/glossary/自動化/)されるわけではありません。特に並行[リクエスト](/glossary/リクエスト/)が多い場合は、ミドルウェアレベルでキューイングや待機ロジックを追加実装することを検討してください。
