...
from fact_check import (
    clear_new_article_failure, evaluate_new_article, record_new_article_failure,
...
def _url_netloc(url: str) -> str:
    """URL のネットロックを小文字で返す（www. プレフィックスは除去）。"""
    try:
...
def generate_methodology_note(row: dict) -> str:
    ...
    all_source_urls = [u.strip() for u in row.get("source_urls", "").split("|") if u.strip()]
    # 取得不可ドメインの URL は引用元として扱わない
    source_urls = [
        u for u in all_source_urls
        if not any(
            _url_netloc(u) == d or _url_netloc(u).endswith("." + d)
...
例（Docker 401 の場合）:
- `401` → HTTP ステータスコード：リクエストは届いたが認証に失敗した
- `Unauthorized` → 理由フレーズ：有効な認証情報がない
- `"message": "authentication required"` → 認証を要求していることを示す JSON フィールド
分解できる構成要素が少ない場合（例: Connection refused）でも、
必ず 1〜2 項目で要素の意味を説明すること。推測で意味を作らず、公式仕様で確認できる定義に基づく。
...
コマンドで確認できない場合（UI の目視確認等）はコードブロックを省略し、
「〇〇の画面で△△と表示されていれば成功です」と 1 文で記述すること。
...
### 6. Editor's Note（H2）【2026-06-29 一時停止中：生成禁止】
このセクションは生成しないこと。ユーザー情報に「参照した実際の報告URL」が含まれていても、
このセクション自体を必ず省略すること。
（以下は一時停止前の指示。人間向け復活メモ、LLMへの指示ではない。
 復活時: source_urls の引用信頼性問題が解決したら、下記を有効な指示に戻すこと。
 旧指示内容: ユーザー情報に「参照した実際の報告URL」が含まれる場合のみ追加。
 形式: 1段落150〜250字。公式解説と実際報告の比較、環境ごとの差異、
 「現場では〇〇が有効」という示唆で締め。URLはMarkdownリンクで1〜2件のみ埋め込む。
 reddit.com・redd.it などクロール制限ドメインのURLは使用しないこと）
...
- reddit.com・redd.it などクロール制限のあるドメインの URL は使用しないこと。
"""
    message = client.messages.create(
        model="claude-haiku-4-5-20251001",
        max_tokens=1200,
...
def _try_generate_article(
    ...
    if fact_result.status == "fact_check_unavailable":
        ...
        remaining.append(row)
        return None, False
    en_match = _EDITOR_NOTE_SECTION_RE.search(body)
    if en_match:
        en_urls = extract_urls(en_match.group(0))
        synthetic_mismatches = []
        for url in en_urls:
            netloc = _url_netloc(url)
            if netloc and any(netloc == d or netloc.endswith("." + d) for d in UNVERIFIABLE_DOMAINS):
                print(f"  [unverifiable_domain] detected: {url}")
                synthetic_mismatches.append({
                    "url": url,
                    "claimed": "",
                    "actual": "このドメインは取得・照合できないため引用源として使用不可",
                })
        if synthetic_mismatches:
            seen_urls = {m["url"] for m in fact_result.citation_mismatches}
            for entry in synthetic_mismatches:
                if entry["url"] not in seen_urls:
                    fact_result.citation_mismatches = list(fact_result.citation_mismatches) + [entry]
                    seen_urls.add(entry["url"])
            print(f"  [unverifiable_domain] {len(synthetic_mismatches)} 件を citation_mismatches に追記")
